...
const RegionSelector = ({ selectedDepartments = [], onChange }) => {
  ...
  const handleToggleDepartment = (code) => {
    ...
    <div className="w-full">
      ...
      <div className="max-h-60 overflow-y-auto border border-gray-300 rounded-md">
        {filteredDepartments.map((dept) => (
          <div 
            key={dept.code}
            className="flex items-center px-3 py-2 hover:bg-gray-100 border-b border-gray-200 last:border-b-0"
          >
            <input
              id={`dept-${dept.code}`}
              type="checkbox"
              checked={selectedDepartments.includes(dept.code)}
              onChange={() => handleToggleDepartment(dept.code)}
